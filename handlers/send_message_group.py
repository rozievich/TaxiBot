...
async def send_message_func(message: types.Message, state: FSMContext):
    await message.answer(
        "E'loningizni yozib qoldiring tez orada taxilar siz bilan bog'lanishadi 🚕\nEslatib o'tamiz bog'lanish uchun <b>telefon raqam</b> yoki <b>telegram username</b> yozib qoldiring 🔗",
...
async def send_message_func_finish(message: types.Message, bot: Bot, state: FSMContext):
    if message.text == "❌":
        ...
        await bot.send_message(chat_id=TO_GROUP_ID,
